...
public class StrathUndergraduate {
	...
	public static final String STRUCTURE_PATH=FILE_PATH+"\\structure_ug\\";
	...
	public static void main(String[] args) {
		try {
			...
			try {
				exportExcel(FILE_PATH, "gen_data_strath_ug.xls");
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public static void run(String originalUrl,int beginIndex) throws Exception{
		...
		Elements links=doc.getElementById("container").getElementsByClass("ug-item");
		...
		for(Element link:links){
			...
			//System.out.println(link.text()+"\t"+baseUrl+link.getElementsByTag("a").get(0).attr("href"));
	...
	public static void getDetails(int row,String url) throws Exception {
		...
		for(TextNode textnode:e.textNodes()){
			if(textnode.text().contains("Assessment")){
				break;
			}
			structure=structure+textnode.text()+"\r\n";
		...
		writeToExcel(row, TUITION_FEE, e.text().substring(e.text().indexOf("International")+"International £".length(),e.text().indexOf("How can I fund my studies?")));
